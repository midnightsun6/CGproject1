...
void Terrian::draw(const Shader& shader) {
    ...
    glBindBuffer(GL_ARRAY_BUFFER, insVBO);
    glBufferData(GL_ARRAY_BUFFER, translation.size() * sizeof(glm::vec2), &translation[0], GL_STATIC_DRAW);
    glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, 2 * sizeof(float), (void*)0);
    glVertexAttribDivisor(2, 1);
    glEnableVertexAttribArray(2);

    glDrawElementsInstanced(GL_TRIANGLES, indices.size(), GL_UNSIGNED_INT, 0, translation.size());
}
